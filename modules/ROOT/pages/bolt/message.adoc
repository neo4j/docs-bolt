...
This message is new in version *5.4*.
...
This message is stored into the metrics system only if telemetry is enabled on both the server and the driver.
If only enabled on the driver, the message will still be sent and a `SUCCESS` will be returned, but nothing will be stored into the server's metrics system.
...
If a `TELEMETRY` message request has been successfully received, the server should respond with a `SUCCESS` and remain in its current state.
...
If a `TELEMETRY` message contains a value that is not a valid `api` value or is sent in the wrong state, the server should respond with a `FAILURE` message and enter the `FAILED` state.
