...
| `01`
| Request
| `extra::Dictionary(user_agent::String, routing::Dictionary(address::String))`
...
If pre version *5.1* then the authentication token described on the `LOGON` message should be sent here on the `HELLO` message instead.
