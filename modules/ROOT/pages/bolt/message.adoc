...
| `01`
| Request
| `extra::Dictionary(user_agent::String, notifications_minimum_severity::String, notifications_disabled_categories::List<String>, routing::Dictionary(address::String))`
...
The `notifications_minimum_severity` specifies the minimum severity a notification needs to have to be returned. Please see the Cypher documentation for acceptable entries.
The `notifications_disabled_categories` is a list of notification categories that will not be returned. Please see the Cypher documentation for available categories.
...
In version 5.2,notifications_minimum_severity::String, notifications_disabled_categories::List<String> where added to be able to control the notification config.
...
** `HELLO`, `BEGIN` and `RUN` messages now accept Notifications config.
