...
** `credentials_expired::Boolean` - the sent credentials are expired, the user must update it.
...
NOTE: This error still as unexpected (`50N42`) since it didn't get migrate yet to GQL standard.

.Example 1 (previous 5.7)
[source, bolt]
----
C: TELEMETRY 2
...
** `db::String?` - user's resolved home database that the transaction will run into. 
...
* `code::String` - the neo4j code identifying the failure. label:deprecated[Removed after bolt 5.6]
* `neo4j_code::String` - the neo4j code identifying the failure. label:new[Introduced in bolt 5.7]
* `gql_status::String` - the gql status which identifies the error. This should be used in favor of `neo4j_code`. See more about GQL Status Code https://development.neo4j.dev/docs/status-codes/current/[here]. label:new[Introduced in bolt 5.7]
* `description::String` - describes the failure represented by `gql_status`. label:new[Introduced in bolt 5.7]
* `diagnostic_record::Dictionary` - contains fields for helping diagnostic the status. Omitted when content is default. label:new[Introduced in bolt 5.7]
...
Those information can be important for diagnosing failures.
