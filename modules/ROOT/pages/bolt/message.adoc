...
| Request
|
| Logs off current user ready for another LOGON message to be sent.
...
| `10`
| Request
| `query::String, parameters::Dictionary, extra::Dictionary(bookmarks::List<String>, tx_timeout::Integer, tx_metadata::Dictionary, mode::String, db:String)`
...
----
HELLO {user_agent::String, patch_bolt::List<String>, scheme::String, routing::Dictionary(address::String))
----
...
.Example 2
----
HELLO {"user_agent": "Example/4.2.0", "patch_bolt": ["utc"], "scheme": "basic", "principal": "user", "credentials": "password", "routing": {"address": "x.example.com:9001", "policy": "example_policy_routing_context", "region": "example_region_routing_context"}}
----
...
If authentication fails, the server must respond with a `FAILURE` message and immediately closes the connection.

[[messages-logoff]]
=== Request message `LOGOFF`

A `LOGOFF` message logs off the currently authenticated user and readies the connection for another `LOGON` message.
