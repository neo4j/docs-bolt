...
An ordered collection of homogeneous types, representing a vector in a multidimensional space.
...
* `type_marker` is specifying the inner type of the vector:
...
* `data` is a sequence of values of the specified type, encoded as specified in the description of `type_marker`.
